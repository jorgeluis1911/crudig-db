class App < Sinatra::Application
  ...
  def load_bd( driver, dominio, usuario, pass, bd_name, port)
    ...
    begin
      ...
      raise( rb_ePGerror , "PQconnectdb() unable to allocate structure")   if(conn == NULL)
  ...
  def select_mysql(select, tabla, params, start_limit)
    ...
    ordenar_sql = ordenar_create(params)
...
=begin    
    consulta = @@conexion.prepare('SELECT '+select+' '+from+' '+where_sql+' '+ordenar_sql +limit)
    consulta.execute
    conulsta.fetch_hash do |row|
      filas["#{cont}"] = row
      cont += 1
    end
    consulta.finish
